...
::: rmdtip
Let's practice importing data, from \@ref(dataframe). Download the [periodic table](http://ismayc.github.io/periodic-table-data.csv) data now, and import it into R as `periodic_table`.
...
Every time the element's state is not `Liquid`, R returns False. Rather than ignore the row, it creates a placeholder saying that the row (which has a non-Liquid element) is not applicable to the query. Although this technically does the job you want, it is not a very elegant solution, which is why the `%in%` operator is better.

...
**XLSX** and **CSV** files are how many datasets are stored. They open through the Excel app. You can tell that they are Excel from its icon: ![](https://gitcdn.link/cdn/kaisamng/RGuides/main/resources/images/00-Getting-Started/XLSX_File.png?display=inline-block)
...
-   To access specific variables inside a data frame, we can do so by entering the name of the data frame followed by `$` and the name of the variable. (Note that spaces in variable names will not work. You'll likely learn that the hard way, as I have.)
...
Recall in \@vector-selection section that we could use the `[ ]` operator, to select elements from a single vector. We can extend these operations into a dataframe. The `[ ]` operator allows you to select elements within a vector or a dataframe **based on its row and column position.** It starts by locating the row position, and then the column position.
...
#### Selecting a single element from a dataframe
...
#### Using Logical Vectors to select from a dataframe

Recall in \@vector-logical-selection that we could use a logical vector, and the `%in%` operator, to select elements from a single vector. We can extend these operations into a dataframe.
